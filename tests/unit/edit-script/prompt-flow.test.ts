...
describe('edit script block-first prompt flow', () => {
  it('builds a screenplay-first prompt chain with style preview candidates before the single confirmed style source', () => {
    ...
    expect(styleBiblePrompt).toContain('hardBans 只写真正的媒介与风格边界')
    expect(styleBiblePrompt).toContain('不要把文字、字幕、水印、logo、标签、符号写入 hardBans')
    ...
    expect(stylePreviewPrompt).toContain('hardBans 只写真正的媒介与风格边界')
    expect(stylePreviewPrompt).toContain('不要把文字、字幕、水印、logo、标签、符号写入 hardBans')
    ...
    expect(englishStylePreviewPrompt).toContain('hardBans should only cover real medium/style boundaries')
    expect(englishStylePreviewPrompt).toContain('Do not include text, subtitles, watermarks, logos, labels, or symbols in hardBans')
