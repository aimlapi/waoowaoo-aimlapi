...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('角色/场景资产与空间档案')
    expect(prompt).toContain('分镜面板与分镜图片')
    expect(prompt).toContain('这是依赖顺序，不是固定话术流程')
    expect(prompt).toContain('修复/重新生成用户正在反馈的当前阶段产物')
    expect(prompt).toContain('时长上限：剪辑先行剧本必须遵守用户选择的时长档位，目前可到 15 分钟档')
    ...
    expect(prompt).toContain('如果用户提交资产修改意见，把它当作当前资产返工')
    expect(prompt).toContain('不依赖 editScript.requirements')
    expect(prompt).toContain('不要传 requirementId')
    expect(prompt).toContain('不要等待导演拆镜、剪辑表、摄影 shot plan 或空间定位准备')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('character/location/prop assets and spatial profiles')
    expect(prompt).toContain('storyboard panels and panel images')
    ...
    expect(prompt).toContain('Duration cap: the edit-first screenplay must follow the user-selected duration tier, currently up to the 15-minute tier')
    ...
    expect(prompt).toContain('If the user submits asset revision notes, treat them as current asset revision work')
    expect(prompt).toContain('does not depend on editScript.requirements')
    expect(prompt).toContain('do not pass requirementId')
    expect(prompt).toContain('do not wait for director decoupage, an edit table, a cinematography shot plan, or spatial-blocking preparation')
  })
  ...
  it('localizes user-facing operation titles without exposing internal ids', () => {
    ...
    expect(localizeProjectAgentOperationTitle('generate_edit_cinematography_shot_plan', 'zh')).toBe('项目操作')
    expect(localizeProjectAgentOperationTitle('generate_edit_cinematography_shot_plan', 'en')).toBe('Project operation')
    expect(localizeProjectAgentOperationTitle('generate_edit_script_storyboard_spatial_blocking', 'zh')).toBe('项目操作')
  ...
  it('describes edit asset generation as screenplay-based without requirement ids', () => {
    ...
    expect(zhDescription).toContain('根据 ready 剧本直接创建/复用')
    expect(zhDescription).toContain('不要要求导演拆镜或剪辑表')
    expect(zhDescription).toContain('不要传 requirementId')
    expect(enDescription).toContain('directly from the ready screenplay')
    expect(enDescription).toContain('Do not require director decoupage or an edit table')
    expect(enDescription).toContain('do not pass requirementId')
  })
