...
describe('platform provider config', () => {
  ...
  it('uses platform models in platform-key mode without user config', async () => {
    ...
    expect(models.map((model) => model.modelKey)).toContain('openrouter::anthropic/claude-fable-5')
    ...
    await expect(resolveModelSelection('user-1', 'openrouter::anthropic/claude-fable-5', 'llm')).resolves.toMatchObject({
      provider: 'openrouter',
