...
describe('buildLocationImagePromptCore', () => {
  it('uses natural invisible layout guidance for available slots in zh prompts', () => {
    ...
    expect(prompt).toContain('生成隔离的空场景资产')
    expect(prompt).toContain('绝对不要出现任何人物、演员、群演、围观者、人形剪影')
    expect(prompt).toContain('最终画面仍必须是无人空场')
  ...
  it('uses natural invisible layout guidance for available slots in en prompts', () => {
    ...
    expect(prompt).toContain('Generate an isolated empty location asset')
    expect(prompt).toContain('Do not include any people, actors, extras, crowds, silhouettes')
    expect(prompt).toContain('keep the rendered scene empty')
