...
describe('project agent operation registry', () => {
  ...
  it('keeps storyboard tools atomic and assistant-visible', () => {
    ...
    expect(registry.generate_edit_director_decoupage).toBeUndefined()
    expect(registry.generate_edit_script).toBeUndefined()
    expect(registry.generate_edit_cinematography_shot_plan).toBeUndefined()
    expect(registry.generate_edit_script_storyboard_spatial_blocking).toBeUndefined()
