...
const criticalCreativeSkillTokens = new Map([
  ...
  ['director-core', ['# 导演与剪辑时间线核心', '`outputKind=video_prompt_set` 时必须同轮读取 `director-core`、`video-direction` 与 `quality-review`', '三份 Skill 服务同一个 strict 结果', '一个 `video_prompt_set.segment` 是一个连续摄影镜头和一次 provider 生成', '每个镜头只承担一个独特的剪辑职责和一个可见动作节拍', '后一镜第一帧必须继承前一镜已经发生的结果', '相邻镜头至少改变景别、机位角度、主体落位中的两项', '除明确的口播、vlog 或打破第四面墙外，不看镜头', '独立生成无法可靠进行帧级半动作接力', '视频镜头默认是完整音画单位']],
  ...
  ['asset-development', ['# 生产资产筛选与稳定身份', '正式 `asset_manifest` 是项目生产资产范围的唯一事实', '本 Skill 不生成图片，不写 provider Prompt，不设计候选', '最终资产图片 Prompt 由服务端把这里的 `stableDescription` 与已采纳 Creative Direction 确定性编译', '`stableDescription` 只保存跨镜头必须稳定', '鞋履和可穿戴配饰', '非人类角色的物种、形态、轮廓', '至少三个能帮助后续人物落位与方向保持的稳定空间锚点', '道具只写本体', 'Worker 不复述这些字段', '每个 asset 只输出']],
  ['video-direction', ['# 视频导演与单镜头生成设计', '`outputKind=video_prompt_set` 时必须在同一轮真实读取 `director-core`、`video-direction` 与 `quality-review`', '一个 `segment` 是一次 provider 生成，也必须是一个连续摄影镜头', '剪辑发生在已生成 segment 之间', '相邻镜头至少在景别、机位角度、主体落位三项中改变两项', '`entryState` / `exitState`', '`incomingCut`', '至少一张图片', '不要输出 `@ImageN`、`@AudioN` 或数据库 ID', '服务端是唯一 Prompt writer', '一个全局唯一 `beatId`', '视频 segment 默认必须有可听原生声音', '需要时间压缩或快切时，把每个 cut 设计成独立 segment', '资产板的白底、左右拼版、正面站姿、居中直视当作剧情构图']],
  ...
  ['quality-review', ['# 创作质量审查', '## Skill 读取组合', '当任务的 `outputKind=video_prompt_set` 时，在创作前必须同时读取 `director-core`、`video-direction` 和 `quality-review`', '不产生独立审查字段或第二份 Prompt', '根据真实可见输入', '服务端会提供完整已采纳方向', '最小范围修正', '稳定实物锚点', '逐对比较前段末镜头与后段首镜头', '景别差异只是必要条件', '是否存在同一个动作节拍被两个镜头分别呈现', '是否出现无第四面墙动机的直视镜头', '是否出现无叙事动机的连续居中构图', '每个 segment 是否恰好一个连续摄影镜头', '需要 cut 时是否已经拆成独立 segment']],
])
