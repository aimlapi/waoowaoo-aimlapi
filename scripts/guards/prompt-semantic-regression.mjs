...
const criticalLocalizedTemplateTokens = new Map([
  ['project-agent/system', {
    en: [
      ...
      'Initiating user brief and expressly supplied references',
      'must not first generate or read this path\'s downstream screenplay to infer style',
      'Delegate exactly one complete `style_bible` Subagent',
      ...
      '`adopt_style_bible`',
      'exactly one `screenplay_draft` Subagent',
      '`00–06T` are internal stages',
      '`materialize_screenplay_draft`',
      'deterministic Stage 07S',
      '`confirm_script_resource`',
      '`canonicalRegistries.characters / locations / props`',
      'Delegate exactly one complete `asset_prompt_set` Subagent',
      'pass its `generationPrompt` directly to `create_image`',
      'Map `canonicalEntity.kind` to `schemaId` exactly',
      '`character` -> `project.character_image`',
      '`location` -> `project.location_image`',
      '`prop` -> `project.prop_image`',
      'never omit `schemaId` or create a generic `other` asset',
      'never omit `schemaId`',
    ...
    zh: [
      ...
      '用户起始创意简报及主动提供的参考',
      '不得先生成或读取本流程下游剧本来反推风格',
      '每个风格版本只委派一个完整 `style_bible` Subagent',
      '用户已经明确给出足够具体的风格时',
      '预览图是可选的收费媒体，不是采用前置。',
      '`adopt_style_bible`',
      '恰好一个 `screenplay_draft` Subagent',
      '`00–06T` 是该同一 Subagent 内部',
      '`materialize_screenplay_draft`',
      '确定性 Stage 07S',
      '`confirm_script_resource`',
      '`canonicalRegistries.characters / locations / props`',
      '只有剧本 Resource 确认后才委派一个完整 `asset_prompt_set` Subagent',
      '把 `generationPrompt` 直接交给 `create_image`',
      '必须把 `canonicalEntity.kind` 精确映射为 `schemaId`',
      '`character` -> `project.character_image`',
      '`location` -> `project.location_image`',
      '`prop` -> `project.prop_image`',
      '不得省略 `schemaId`，也不得产生通用 `other` 资产',
      '不得省略 `schemaId`',
...
const criticalCreativeSkillTokens = new Map([
  ...
  ['story-development', {
    en: ['# Story Development', 'Runtime comes from dialogue, action, reaction, pauses, and transitions', 'Scene body text must be a complete filmable scene', '`sourceType=external_hook`', '`laterLinkBackBeatId`', 'reuses the complete source payload'],
    zh: ['# 故事与剧本开发', '时长来自对白、动作、反应、停顿和转场的实际时间', '真正可拍摄的完整场景', '`sourceType=external_hook`', '`laterLinkBackBeatId`', '逐字段复用完整 source payload'],
  }],
  ['continuity-memory', {
  ...
  ['style-development', {
    en: ['# Visual Style Development', '## Input boundary', 'Never generate a screenplay first', '`visualStyle` is the shared image/video look', '`assetImageStyle` is used only for asset images', 'Fixed asset-image formats do not belong to the Style Bible', 'Video composition remains independently owned by directing and video design', 'Candidates must not choose or generate an asset-image format', 'Candidates must differ materially', 'must not change character identity', 'The Style Bible is the sole authority for visual style', 'not a closed enum', 'never let catalog vocabulary rewrite those facts', 'at least two visual dimensions', 'Translate every selected label into executable'],
    zh: ['# 视觉风格开发', '## 输入边界', '不得先生成剧本、读取本流程下游生成的剧本', '`visualStyle` 是图片和视频共享的总体画面风格', '`assetImageStyle` 只供角色图', '资产图固定版式不属于 Style Bible', '视频构图仍由导演与视频模块独立决定', '候选不得选择或生成资产图版式', '候选应在美术媒介、总体质感、色彩与设计语言上形成实质差异', '不得借预览改变人物身份', 'Style Bible 是视觉风格的唯一权威', '不是封闭枚举', '素材词也不得改写这些事实', '至少在两个视觉维度上有实质差异', '把选中的词转译成具体可执行的媒介'],
  }],
  ['asset-development', {
    en: ['# Asset Development and Generation Prompts', 'sole and exhaustive asset list', 'Add none, omit none, duplicate none, merge none', '`stableDescription`', '`generationPrompt`', '## Fixed asset-image formats and execution boundary', 'fixed 4:3 landscape image', 'equal left and right halves', 'complete panoramic environment seen straight on', 'shows exactly one prop', 'do not own, override, or constrain video composition', 'Shoes are mandatory', '### Non-human characters', 'Do not use uncertainty', 'at least three stable, clearly visible structural anchors', 'clear, sharp, richly detailed, and production-quality', 'foundational location description stored as project fact', "Describe only the prop's static visible body", 'Preserve every unmodified identity'],
    zh: ['# 资产设计与生成提示词', '唯一、穷尽的资产名单', '不得新增、遗漏、重复、合并', '`stableDescription`', '`generationPrompt`', '## 固定资产图格式与执行边界', '固定 4:3 横向图片', '左右各半', '从正前方完整展示空间的全景环境图', '只展示一个摆正且完整可见的道具', '不拥有、覆盖或约束视频构图', '鞋子是完整人物设计的必要部分', '### 非人类角色', '不用“或”“可能”“也许”“大概”等不确定词', '至少三个稳定、清晰可见的结构锚点', '清晰锐利、细节丰富并达到专业生产质量', '作为项目事实保存的基础地点描述', '只描述道具本体的静态视觉信息', '保留所有未被修改的原有身份'],
  }],
  ['video-direction', {
