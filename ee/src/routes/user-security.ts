...
export const handleUserSecurityGet = apiHandler(async () => {
...
export const handleUserSecurityPost = apiHandler(async (request: NextRequest) => {
  const authResult = await requireUserAuth()
  if (isErrorResponse(authResult)) return authResult
...
export const handleUserSecurityPatch = apiHandler(async (request: NextRequest) => {
  const authResult = await requireUserAuth()
  if (isErrorResponse(authResult)) return authResult
  readAccountSecurityFeatures()
