...
if [ ! -f "$release_dir/Dockerfile" ] \
  ...
  || [ ! -f "$release_dir/ee/scripts/production/host-maintenance.sh" ] \
  || [ ! -f "$release_dir/ee/scripts/production/registry-maintenance.sh" ] \
  || [ ! -f "$release_dir/ee/scripts/production/install-local-registry.sh" ] \
  || [ ! -f "$release_dir/ee/scripts/production/install-host-maintenance.sh" ] \
  || [ ! -f "$release_dir/ee/scripts/production/mysql-backup-maintenance.sh" ] \
...
WAO_REGISTRY_RECREATE=1 \
  sh "$release_dir/ee/scripts/production/install-local-registry.sh" --apply
sudo -n true
...
docker build \
  --build-arg DEPLOYMENT_EDITION=cloud \
...
WAO_DEPLOY_ROOT="$deploy_root" \
  sh "$release_dir/ee/scripts/production/mysql-backup-maintenance.sh" --backup-only
...
WAO_DEPLOY_ROOT="$deploy_root" \
  sh "$release_dir/ee/scripts/production/host-maintenance.sh" --apply
WAO_DEPLOY_ROOT="$deploy_root" \
  sh "$release_dir/ee/scripts/production/registry-maintenance.sh" --apply
sudo -n sh "$release_dir/ee/scripts/production/install-host-maintenance.sh" "$deploy_root"
rm -rf "$release_dir"
