...
if ! sh "$repo_root/ee/scripts/production/install-local-registry.sh" --check; then
...
install -m 0755 "$repo_root/ee/scripts/production/host-maintenance.sh" \
  /usr/local/lib/waoowaoo/host-maintenance.sh
install -m 0755 "$repo_root/ee/scripts/production/registry-maintenance.sh" \
  /usr/local/lib/waoowaoo/registry-maintenance.sh
install -m 0755 "$repo_root/ee/scripts/production/mysql-binlog-maintenance.sh" \
  /usr/local/lib/waoowaoo/mysql-binlog-maintenance.sh
install -m 0755 "$repo_root/ee/scripts/production/mysql-backup-maintenance.sh" \
  /usr/local/lib/waoowaoo/mysql-backup-maintenance.sh
install -m 0644 "$repo_root/ee/ops/systemd/waoowaoo-host-maintenance.service" \
  /etc/systemd/system/waoowaoo-host-maintenance.service
install -m 0644 "$repo_root/ee/ops/systemd/waoowaoo-host-maintenance.timer" \
