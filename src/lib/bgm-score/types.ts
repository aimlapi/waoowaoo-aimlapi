...
export const scoreCandidateQualitySchema = z.object({
  ...
  spectralDistribution: z.object({
    sub: z.number().min(0).max(1),
  ...
  spectralBudgetError: z.number().min(0).max(1),
  spectralCoverage: z.number().int().min(0).max(7),
  timbralVariation: z.number().min(0).max(1),
  crestFactorDb: z.number().min(0),
  dynamicRangeDb: z.number().min(0),
...
export const bgmScoreProjectDataSchema = z.object({
  ...
  scoreCandidates: z.array(scoreCandidateAssetSchema).max(4).optional(),
