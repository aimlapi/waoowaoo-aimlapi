...
export function buildLocationImagePromptCore(params: {
  ...
  const emptySceneConstraints = params.locale === 'en'
    ? 'Generate an isolated empty location asset. Do not include any people, actors, extras, crowds, silhouettes, reflections of people, portraits, human figures on screens, hands, body parts, or character stand-ins anywhere in the image. If the description mentions people only to explain later blocking or story context, treat that as context only and keep the rendered scene empty.'
    : '生成隔离的空场景资产。画面里绝对不要出现任何人物、演员、群演、围观者、人形剪影、人物倒影、照片肖像、屏幕里的人像、手部、身体局部或角色替身。如果描述里提到人物只是为了说明后续调度或剧情语境，只能当作背景信息，最终画面仍必须是无人空场。'
