...
export function buildStoryboardStillPrompt(input: {
  ...
  return [
    ...
    'If REFERENCE_IMAGES contains same production location visual memory or previous storyboard panels, use them only for permanent spatial footprint: fixed set dressing, anchor adjacency, separation distance, intervening objects, and screen direction.',
    'Do not copy narrative state from references: character pose, temporary props, table contents, phone screens, time state, crowd state, or accidental contamination must come from this panel only.',
    'Close-ups and inserts must preserve implied room depth. Do not collapse separated objects into contact; if a bed edge, aisle, doorway, wall gap, counter, or foreground distance separates two anchors, keep that separation visible or clearly implied.',
    '',
    'SHOT_PRIORITY',
