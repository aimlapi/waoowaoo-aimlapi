...
export function buildStoryboardGridPrompt(input: {
  ...
  return [
    ...
    'If REFERENCE_IMAGES contains same production location visual memory, use it only for permanent spatial footprint: fixed set dressing, anchor adjacency, separation distance, intervening objects, and screen direction.',
    'Do not copy narrative state from same-location references: character pose, temporary props, table contents, phone screens, time state, crowd state, or accidental contamination must come from the current cell only.',
