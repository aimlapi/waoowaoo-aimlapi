...
export const CHARACTER_PROMPT_SUFFIX = '角色资产参考图，画面分为左右两个区域：【左侧区域】占约1/3宽度，是角色的正面特写（如果是人类则展示完整正脸，如果是动物/生物则展示最具辨识度的正面形态）；【右侧区域】占约2/3宽度，是角色三视图横向排列（从左到右依次为：正面全身、侧面全身、背面全身），三视图高度一致。画面只能出现当前这一个角色，绝对不要出现其他人物、围观者、路人、替身、同伴、儿童、手部代入、照片/镜子/屏幕里的人像或人形剪影。背景、光线、材质、色彩和画面质感只能来自当前剧本角色描述与已选视觉参考案例，不要引入纯白底、摄影棚、旧项目风格、系统风格预设或额外画风。不要生成文字标签、编号、水印或 Logo。'

// 道具图片生成的系统后缀（三视图资产图，风格由剧本与视觉参考案例决定）
export const PROP_PROMPT_SUFFIX = '道具资产参考图，画面分为左右两个区域：【左侧区域】占约1/3宽度，是道具主体的主视图特写；【右侧区域】占约2/3宽度，是同一道具的三视图横向排列（从左到右依次为：正面、侧面、背面），三视图高度一致。主体必须完整清楚，材质、磨损、年代感、光线、色彩和画面质感只能来自当前剧本道具描述与已选视觉参考案例，不要引入纯白底、摄影棚、旧项目风格、系统风格预设或额外画风。不要添加与剧本无关的人物、手部、桌面陈设、环境背景、文字标签、编号、水印或 Logo。'
