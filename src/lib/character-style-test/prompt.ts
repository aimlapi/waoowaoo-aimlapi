...
function buildChineseCastingCandidateBrief(candidateIndex: number | undefined): string {
  ...
  const briefs = [
    [
      '候选 A 方向：生活真实度优先。',
      '这是一位独立候选演员脸：脸骨和五官比例要偏普通真实、克制、接近现实中会遇到的人；服装以日常基础层次和真实旧衣质感为主。',
      '必须与候选 B/C 拉开：不要共用同一张脸、同一头模或同一底模，不要使用强戏剧化眼神、明显造型发型、强配饰或高度记忆点妆造。',
    ].join(' '),
    [
      '候选 B 方向：情绪创伤与表演强度优先。',
      '这是一位独立候选演员脸：脸部骨相、眼神湿度、疲惫感、脆弱表情和内在压抑要明显强于 A/C；服装可以更松垮、磨损或带生活崩塌感。',
      '必须与候选 A/C 拉开：不要只是同一张脸换表情，不要沿用 A 的日常中性妆造或 C 的造型记忆点。',
    ].join(' '),
    [
      '候选 C 方向：造型记忆点与轮廓识别优先。',
      '这是一位独立候选演员脸：在仍然适合作为角色定妆候选的范围内，强化不同头颅轮廓、发型轮廓、服装层次、标志性配饰、身体标记或独特穿搭比例。',
      '必须与候选 A/B 拉开：脸型气质、五官比例、发型、服装结构和识别点都要明显不同，不能只是同一人物的第三张相似 contact sheet。',
    ].join(' '),
  ] as const
...
function buildEnglishCastingCandidateBrief(candidateIndex: number | undefined): string {
  ...
  const briefs = [
    [
      ...
      'This is an independent actor-face candidate: facial bone structure and feature proportions should feel ordinary, restrained, and close to someone encountered in real life; wardrobe uses practical everyday layers and worn real clothing.',
      'Differentiate from B/C: do not share the same face, head mold, or base model, and do not use highly dramatic gaze, statement hair, strong accessories, or memorable styling marks.',
    ].join(' '),
    [
      'Candidate B direction: emotional wound and performance intensity first.',
      'This is an independent actor-face candidate: facial structure, wet gaze, fatigue, vulnerable expressions, and suppressed inner pressure should be much stronger than A/C; wardrobe may feel looser, worn, or life-collapsed.',
    ...
    [
      ...
      'This is an independent actor-face candidate: still realistic and castable, but with stronger skull silhouette, hair silhouette, wardrobe layering, signature accessory, visible body mark, or distinctive proportion.',
      'Differentiate from A/B: face impression, facial proportions, hair, costume structure, and visual hook must be visibly different, not a third near-identical contact sheet of the same generated person.',
    ].join(' '),
  ] as const
  return briefs[candidateIndex] ?? ''
}
...
function buildChineseCastingPhotoPrompt(
  ...
  return [
    ...
    '当前这张图是一位候选演员的完整形象包：整张图必须是一张拼版，包含当前候选演员身份的多张定妆参考；不是单纯白底三视图，也不是多个不同角色拼在一起。',
    ...
    '单张候选包内一致性规则：只有当前这张候选图内部的多格需要保持同一个候选演员身份稳定；脸型、五官距离、鼻梁、颧骨、眼神、体型、身高比例、发量和主要身体标记必须稳定；允许服装、情绪、背景和局部妆造变化，但不能在同一张候选包内部换人。',
...
function buildEnglishCastingPhotoPrompt(
  ...
  return [
    ...
    'This is one complete candidate actor look package: the image must be one collage containing multiple look-test views of the current candidate actor identity. It is not a plain white-background turnaround only, and it must not mix multiple different characters.',
    ...
    'Within-sheet consistency rule: only the panels inside this current candidate image must preserve one candidate actor identity. Face shape, feature spacing, nose bridge, cheekbones, gaze, body type, height proportion, hair volume, and key body marks must stay stable. Wardrobe, emotion, background, and small makeup/styling changes may vary, but the person must not change inside this one sheet.',
