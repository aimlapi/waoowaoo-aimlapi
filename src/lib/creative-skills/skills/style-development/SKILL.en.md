...
Translate the user's initiating creative brief, explicit style direction, and user-supplied factual or visual references into one visual-style contract shared by images and video, with materially distinct style candidates and preview directions when needed. This Skill governs visual style only; it does not design the specific identity of characters, locations, or props.

## Input boundary

- In the complete `idea -> style -> screenplay` path, this Skill consumes only the initiating user brief, explicit user style direction, and factual or reference material the user supplied before style selection.
- Never generate a screenplay first, read a screenplay generated downstream in this path, or infer the current candidates from a confirmed screenplay. Only after style adoption does the exact Style Revision flow downstream to screenplay as read-only context.
- A complete screenplay explicitly supplied by the user may be an initiating source. It remains user input, not a downstream screenplay generated by the system in order to choose style.
- Never auto-read the project's `confirmed_screenplay` Binding, and never accept a formal `project.source_script` Resource as this `style_bible` run's source. Restyling an existing screenplay needs a separate explicit contract; this path must not infer an implicit “latest screenplay.”
...
- Every style candidate remains faithful to the plot, character, location, prop, and time facts already explicit in the user's input.
...
1. Lock the plot, character, location, prop, era, and world facts explicit in the initiating user brief and user-supplied references. Do not wait for or read a downstream generated screenplay, and never let catalog vocabulary rewrite those facts.
