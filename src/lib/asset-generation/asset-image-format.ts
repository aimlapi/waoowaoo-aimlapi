...
/**
 * Asset-only still-image format authority. This policy does not apply to
 * generic images, style previews, storyboards, video frames, or video prompts.
 */
...
function hasCurrentAssetImageFormatPolicy(input: {
  readonly prompt: string | null | undefined
  readonly aspectRatio: unknown
  readonly kind: AssetImageKind
}): boolean {
  if (input.aspectRatio !== getAssetImageFormatPolicy(input.kind).aspectRatio) return false
  const prompt = input.prompt?.trim()
  if (!prompt) return false
  return Object.values(getAssetImageFormatPolicy(input.kind).instruction)
    .some((instruction) => prompt.endsWith(instruction))
}

export function hasCurrentFrozenAssetImageFormatPolicy(input: {
  readonly kind: AssetImageKind
  readonly taskPrompt: string | null | undefined
  readonly taskAspectRatio: unknown
  readonly resourcePrompt: string | null | undefined
  readonly resourceAspectRatio: unknown
}): boolean {
  return hasCurrentAssetImageFormatPolicy({
    prompt: input.taskPrompt,
    aspectRatio: input.taskAspectRatio,
    kind: input.kind,
  }) && hasCurrentAssetImageFormatPolicy({
    prompt: input.resourcePrompt,
    aspectRatio: input.resourceAspectRatio,
    kind: input.kind,
  })
}
