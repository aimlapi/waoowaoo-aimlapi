...
export function buildCharacterCandidatePromptInstruction(input: {
  ...
  if (input.locale === 'en') {
    return joinLines([
      ...
      'Generate three distinct image-generation prompts for the same character. These prompts will be used to create three candidate character asset images, then a user will choose one default reference.',
      ...
      '- Each prompt must be a final image prompt fragment only: character identity, silhouette, age impression, body type, face/hair direction, clothing design, wearable accessories, and material texture.',
      ...
      '- If a Style Bible is provided, make the casting, clothing, palette, and texture compatible with it inside the prompt wording.',
      '- Do not include layout, background, aspect ratio, image count, multi-view, action, or context-sample instructions. The execution policy applies the fixed character-asset format later.',
  ...
  return joinLines([
    ...
    '请为同一个角色生成三条不同的最终图片生成提示词。这三条提示词会分别生成三张候选人物资产图，之后由用户选择一张作为默认参考。',
    `基础角色输入：${description}`,
    styleBlock ? `项目 Style Bible：\n${styleBlock}` : null,
    '规则：',
    '- 必须输出同一个角色身份的三组形象候选，不是三个不同角色。',
    '- 每条 prompt 只能是最终生图提示词片段：角色身份、轮廓、年龄感、体型、五官/发型方向、服装设计、可穿戴配饰和材质质感。',
    '- 给生图模型保留合理发挥空间。除非输入明确要求，不要把每一个五官细节、发丝、色块、配饰都写死。',
    '- 三条 prompt 必须侧重点不同：1）身份与轮廓忠实版，2）服装/材质/年代质感版，3）角色能量与分镜可用性版。',
    '- 如果存在 Style Bible，必须把选角、服装、色彩和材质写成与它兼容的视觉描述。',
    '- 不要写版式、背景、画幅比例、图片数量、多视图、动作或语境样本；固定人物资产图格式由执行 policy 统一追加。',
