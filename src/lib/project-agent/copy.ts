...
export function buildProjectAgentSystemPrompt(params: {
  ...
  if (params.locale === 'en') {
    return [
      ...
      'For character assets, create or reuse the real ProjectCharacter and primary CharacterAppearance first, then call generate_character_casting_test with promptMode=casting_photo and the appearanceId. This operation generates exactly three casting/contact-sheet candidate packages, scores them, and writes the winning candidate back to the appearance.',
      ...
      'For current assets: create/reuse real ProjectCharacter, ProjectLocation, and prop records directly from the screenplay, visual references, and user requirements. Character assets should use generate_character_casting_test after the character appearance exists; use generate_character_image/generate_location_image only when you already have a real ProjectCharacter/ProjectLocation ID and need non-casting asset renders.',
  ...
  return [
    ...
    '生成人物资产时，先创建或复用真实 ProjectCharacter 与主 CharacterAppearance，再调用 generate_character_casting_test，并传 promptMode=casting_photo 与 appearanceId。该操作固定生成三组选角/定妆候选包，自动按剧本匹配度、身份辨识度、素材完整度等维度评分，并把最高分候选写回该角色形象。',
    ...
    '当前资产链路里，应根据剧本、视觉风格参考和用户要求直接创建/复用真实 ProjectCharacter、ProjectLocation 与道具资产。角色先在已有主形象 appearanceId 上调用 generate_character_casting_test，生成三组选角定妆候选并自动选择最高分；只有已经拿到真实 ProjectCharacter/ProjectLocation ID 且需要非选角资产图时，才调用 generate_character_image/generate_location_image。',
