...
function buildRouterPrompt(params: {
  ...
  if (params.locale === 'en') {
    return {
      system: [
        ...
        'For current production requests such as short-film generation, assets, storyboard, video blocks, or final render, prefer direct asset/storyboard/media groups as needed: ["extra"], ["project","data"], ["asset"], ["storyboard","edit"], ["media","video"], ["media","music"]. Request ["edit-script"] only for screenplay generation or when the user explicitly asks for an edit table.',
  ...
  return {
    system: [
      ...
      '对于当前制作请求，例如生成短片、资产、分镜、视频片段或最终成片，按需要优先请求直接资产/分镜/媒体 group：["extra"]、["project","data"]、["asset"]、["storyboard","edit"]、["media","video"]、["media","music"]。只有生成剧本或用户明确要求剪辑表时，才请求 ["edit-script"]。',
