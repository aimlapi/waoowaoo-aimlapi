...
export function buildEditAssetDesignInstruction(input: BuildEditAssetDesignInstructionInput): string {
  return JSON.stringify({
    ...
    constraints: [
      ...
      'The asset description must apply the shared visual style plus asset-only lighting and texture rules.',
      'Do not invent an asset-image layout. The asset generation execution policy owns the fixed format for each asset kind.',
