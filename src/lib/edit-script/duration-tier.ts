...
const EDIT_FIRST_DURATION_SPECS: Record<EditFirstDurationTier, EditFirstDurationSpec> = {
  ...
  fifteen_min: {
    ...
    zhLabel: '15 分钟',
    enLabel: '15 minutes',
    zhGuidance: '十五分钟档位，约 900 秒。允许完整短片结构、充分场景推进和角色关系递进；最终总时长可按叙事自然落在约 13-16 分钟。',
    enGuidance: 'Fifteen-minute duration tier, around 900 seconds. Allow a complete short-film structure, fuller scene progression, and relationship escalation; the final total duration may naturally land around 13-16 minutes.',
  },
}
...
const EDIT_FIRST_SCENE_COUNT_SPECS: Record<EditFirstDurationTier, EditFirstSceneCountSpec> = {
  ...
  fifteen_min: {
    ...
    minScenes: 10,
    maxScenes: 14,
    zhGuidance: 'Scene Layer 必须为 10-14 场，推荐 12 场。',
    enGuidance: 'The Scene Layer must contain 10-14 scenes; target 12 scenes.',
  },
}
...
function durationSecondsToTier(seconds: number): EditFirstDurationTier | null {
  ...
  if (seconds >= 780 && seconds <= 960) return 'fifteen_min'
  return null
}
...
export function readEditFirstDurationTierFromText(text: string): EditFirstDurationTier | null {
  ...
  if (/(15\s*分钟|十五分钟|fifteen[- ]?minute|15[- ]?minute)/i.test(normalized)) return 'fifteen_min'
