...
export const EDIT_FIRST_DURATION_TIERS = ['short', 'medium', 'long', 'fifteen_min'] as const
...
const EDIT_FIRST_DURATION_SPECS: Record<EditFirstDurationTier, EditFirstDurationSpec> = {
  ...
  fifteen_min: {
    ...
    zhLabel: '15 分钟',
    enLabel: '15 min',
    zhGuidance: '15 分钟档位，约 900 秒，硬上限 900 秒。允许完整短剧结构、多段推进和更充分的人物转折，但最终总时长不得超过 15 分钟，不要为了凑时长填充冗余情节。',
    enGuidance: '15-minute duration tier, around 900 seconds with a hard 900-second cap. Allow a complete short-drama structure, multiple progressions, and fuller character turns, but the final total duration must not exceed 15 minutes and must not add filler just to use time.',
  },
}
...
function parseNumberedDurationSeconds(text: string): number | null {
  ...
  if (/十五分钟/.test(normalized)) return 900
...
function durationSecondsToTier(seconds: number): EditFirstDurationTier | null {
  ...
  if (seconds <= 900) return 'fifteen_min'
  return null
}

export function isEditFirstDurationTier(value: unknown): value is EditFirstDurationTier {
  return value === 'short' || value === 'medium' || value === 'long' || value === 'fifteen_min'
}
...
export function readEditFirstDurationTierFromText(text: string): EditFirstDurationTier | null {
  ...
  const structuredMatches = Array.from(normalized.matchAll(/(?:时长档位|duration tier)\s*[:：]?\s*(short|medium|long|fifteen_min)\b/g))
  ...
  if (/(15\s*分钟|15\s*min|15\s*minutes|十五分钟|fifteen[_ -]?min|fifteen[_ -]?minute)/i.test(normalized)) return 'fifteen_min'
...
export function buildEditFirstStructuredUserPrompt(input: {
  ...
  const structuredLine = input.locale === 'en'
    ? `Structured edit-first parameters: duration tier ${spec.tier} (${spec.enLabel}, around ${String(spec.targetSeconds)} seconds); split screenplay scenes only by actual story location and time, not by dramatic phase count. Final aspect ratio ${input.aspectRatio}.`
    : `剪辑先行结构化参数：时长档位 ${spec.tier}（${spec.zhLabel}，约 ${String(spec.targetSeconds)} 秒）；剧本场景只按故事实际发生地点和时间段拆分，不按剧情阶段凑场数。最终画面比例 ${input.aspectRatio}。`
