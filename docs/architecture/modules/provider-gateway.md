...
- **PG-16 — Voice Design 是独立固定模态。** `voice` 不能伪装成 music 或 video audio。Qwen Voice Design 1.7B 的模型 identity、多语言枚举和按字符价格由 FAL production catalog 唯一声明；Agent-facing `generate_voice` 不接受 provider/model/temperature/seed 等参数。Operation plan 必须在创建收费 Task 前用当前用户真实 runtime catalog 与 provider config 解析同一固定 identity，不能把配置缺失推迟到 worker。adapter 只映射 canonical `description + text + language` 为 FAL `prompt + text + language`，通过共享 FAL external id/poll 协议恢复，完成后仍进入共享音频字节上限和稳定 Task artifact 持久化。
...
- Qwen Voice Design 首次接入只登记了 capability、pricing、adapter 和固定模型选择，却遗漏 platform/API runtime catalog，并让 user-key runtime 保留第二份不含 `voice` 的媒体类型判定；首次修正虽然让生产 catalog 可解析 voice，却仍只在 worker 执行时验证当前 self-host 用户配置。真实环境的 `voiceModels=[]` 于是继续创建收费计划、Resource 与 Task，随后才以 `MODEL_NOT_FOUND ... not enabled for voice` 失败；provider contract 因 mock runtime-config 未能反证。当前 Operation plan 复用真实 `resolveModelSelection + getProviderConfig` 做前置验证，配置缺失不产生 Task；adapter 与 worker 不增加 fallback。
