...
- **CR-03A — Style Bible 是结构化文字 Resource。** finalized Worker 风格结果只经 `adopt_style_bible` 写成 `mediaType=text + schemaId=project.style_bible` 的不可变 revision；Task id 是专业 origin，Worker 输入 Resource revision 是 lineage。候选结果不得写入该 schema。Canvas 使用 schema-aware Style Bible renderer 展示摘要、视觉表达、资产图光线与材质，不把结构化内容退化成 raw JSON；资产图固定版式不存入 Style Bible，也不影响视频构图。这仍是通用 ResourceCard 的专业变体，不建立第二实体。
...
- **CR-13 — Resource Tool 参数只表达产品语义。** `schemaId` 必须从按 mediaType 穷尽分组的生产 Resource registry 生成 enum；通用语义用对应 `generic.*`，不得允许模型发明新 identity。`create_image` 的新建分支必须显式提交 `schemaId`；`asset_prompt_set.semanticKind` 中的 character/location/prop 必须分别映射为对应 `project.*_image`，只有真正的 other 才映射 `generic.image`，禁止通过省略字段默认成 generic 绕过资产图 policy。文本结果以 `single|candidates` 分支表达；异步媒体的全部新创作参数只存在于 `request.kind=new`，`request.kind=retry` 只接受失败 Resource identity。Agent-facing 图片/视频/音乐字段使用跨 provider 产品名，且不得包含 `modelKey`、`*Model` 或 provider 参数；实际模型由服务端正式配置唯一解析。`generationOptions` 只是在批准计划与 Revision provenance 中冻结的内部执行快照，不是公开 Tool 参数。服务端解析出的 modelKey 对应 capability registry 是允许字段和值的唯一裁判，不支持必须 typed-fail，禁止跨模型默认或 provider 参数透传。
