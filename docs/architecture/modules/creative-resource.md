...
- **CR-03A — Creative Direction 是单一结构化文字 Resource。** 每个 Creative Task 终态恰好物化一个 `mediaType=text + schemaId=project.creative_direction` 的不可变 Revision；内容严格为 `styleSummary/rawUserStyle + visual/narrative/directing/editing/sound/assetPolicy`，其中 `visual` 必须显式包含 `renderMedium/realismLevel/crossMediaStyle/visualStyle/assetImageStyle(lighting/texture/renderingRules)`，规则与禁止项归入所属领域。Task id 构成稳定 origin，实际输入 revisions 构成 lineage；专业输出不声明 candidate key 或候选集合。`adopt_creative_direction` 不复制内容，只把该 Task 产出的精确 Revision 写入 canonical Binding。Canvas 使用 schema-aware renderer 展示结构化内容，不建立第二实体或默认预览图。
...
- **CR-12 — Resource 不裁决固定创作顺序，但执行前置条件必须显式。** Resource 存在、缺失、旧 Revision 或 Lineage 不形成 Workflow step。Operation 是否可调用由 registry channel、显式输入 schema、owner/scope、schema generation policy、provider capability、计费批准和破坏性确认裁决；例如项目资产图要求 adopted Manifest 与 Direction，角色音色要求该角色已有 ready 资产图，`project.video_segment` 要求已绑定项目资产图片与原生音轨，这些是当前动作的输入完整性，不是固定阶段状态。Canvas 位置和“上游是否推荐完成”不得成为硬门。
...
- **CR-22 — 剧本与资产清单分权。** 用户在当前消息提供的完整剧本可由 `create_text.current_user_text` 以 `classification.kind=screenplay` 直接保存为项目级 `project.screenplay` Revision；这一步只做精确来源捕获，不创作、不改写、不确认，也不自动委派 Worker。从零创作或修改剧本由 `story-development + creative_work(outputKind=screenplay)` 完成，结果同样只拥有文本和写作元信息。`asset-development` 独占生产资产范围：它在一个 `asset_manifest` Subagent Task 内读取精确 screenplay，完成必要角色、地点、道具的筛选、归一与无风格稳定身份设计；不输出最终媒体 Prompt。精确 screenplay Revision 与 Resource Lineage 记录本次判断实际消费的来源，服务端只编译稳定 `manifestAssetId` 并验证名称与别名歧义，不要求 Worker 回传逐字 excerpt、字符区间或逐项来源证明，也不建立第二套资产状态。
- **CR-23 — 专业媒体生成 policy 由 schema registry 声明。** `project.character_image/location_image/prop_image` 的最终 Prompt 只由 `compileAssetImagePrompt` 从精确稳定身份和 adopted Direction 编译，Agent-facing asset 请求不接受 prompt、画幅或模型参数。`project.video_segment` 在同一 registry 声明 `projectAssetImageReference=required + nativeAudio=required`；计划和 retry 都从该声明校验每张图片 revision 的正式 `project_asset_image` Binding，并把 `generateAudio=true` 冻结。通用 image/video schema 不继承这些要求，调用方不得按 schema 名写分支猜测。
- **CR-24 — 资产采用与图片生成分离。** `adopt_asset_manifest` 是项目资产 identity 的唯一批量采用入口；它通过共享 Project asset writer 创建或复用 `ProjectCharacter/ProjectLocation`，只写 `adopted_asset_manifest` Binding，不创建图片 Task。图片仍只能由 `create_image` 生成；`project.character_image/location_image/prop_image` 在计划阶段由单一 asset-image format policy 强制 4:3 固定后缀，完成后才以 `project_asset_image + variantId` Binding 关联目标资产。Creative Direction 不包含 composition，generic image 不消费该 policy。
