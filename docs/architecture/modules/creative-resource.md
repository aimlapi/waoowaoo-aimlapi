...
- **CR-03A — Style Bible 是结构化文字 Resource。** finalized Worker 风格结果只经 `adopt_style_bible` 写成 `mediaType=text + schemaId=project.style_bible` 的不可变 revision；Task id 是专业 origin，Worker 输入 Resource revision 是 lineage。风格 Task 的 Resource 来源必须在提交前按精确 revision/fingerprint/scope 重读真实 `contentText/contentJson`，不得把调用方自报 content 记成真实 lineage；正式 `project.source_script`、domain provenance 与没有可重读文字／结构内容的 Resource 都不能进入该风格 Task。候选结果不得写入该 schema。Canvas 使用 schema-aware Style Bible renderer 展示摘要、视觉表达、资产图光线与材质，不把结构化内容退化成 raw JSON；资产图固定版式不存入 Style Bible，也不影响视频构图。这仍是通用 ResourceCard 的专业变体，不建立第二实体。
- **CR-03B — 剧本 Resource 只由 completed Task 确定性物化。** `materialize_screenplay_draft` 是 `screenplay_draft` strict 结果进入 `mediaType=text + schemaId=project.source_script` 的唯一入口。它只接受 payload/result request identity 与当前 Operation user/project/episode scope 均精确一致的 completed `creative_work_v4` Task，重新解析完整 06T-valid result，并核对 result 中的 Style Revision 与 Task payload 中服务端冻结的 adopted Style 完全一致；Task 中声明 Resource provenance 的来源材料必须已由 production-context compiler 重读真实 Revision 内容并校验 fingerprint/scope，且不能夹带第二份 Style Bible，不能信任调用方拼接的 content。随后在本地运行无模型的 07S compiler，按呈现顺序保留每个 kernel 恰好一次，只把连续同场景 kernel 分为同一 scene instance，离开后再次返回形成新 instance。它追加包含完整 `canonicalRegistries`、场景/实体引用、状态账本、kernel、compiler metadata 与可读剧本文本的结构化 Revision，并保存 Task provenance、Style Revision 与实际来源材料 lineage；不得增删创意内容，也不得确认 Binding。
...
- **CR-11 — 候选是同一调用的独立资源。** 一次 count=N 的调用必须产生 N 个稳定 candidate Resource，并共享 candidateSetId；成功候选不会因兄弟失败而重提。Agent 重试必须显式引用失败 resourceId，只提交失败候选；retry Tool 分支不得接收 prompt、references、时长、画幅、声音或 provider 参数，服务端只能从该 Resource 第一次 `request.kind=new` 的失败 Task 恢复完整冻结 payload，并以新的执行 identity 重提同一内容。专业角色、场景、道具图片的冻结 Prompt 尾部和顶层/Resource 比例必须与当前资产图 policy 完全一致；旧版式失败任务必须 typed-fail 并要求新建生成，不得改写冻结输入或静默重用。同一 provider invocation 的至多一次保证继续由 Task checkpoint 负责，不建立第二套 logicalInvocationId 或 outcome 协议。
...
- **CR-13 — Resource Tool 参数只表达产品语义。** `schemaId` 必须从按 mediaType 穷尽分组的生产 Resource registry 生成 enum；通用语义用对应 `generic.*`，不得允许模型发明新 identity。`create_image` 的新建分支必须显式提交 `schemaId`；正式 `asset_prompt_set` 的 `canonicalEntity.kind` 只允许 character/location/prop，并必须分别映射为对应 `project.*_image`，不得产生 `other`、映射成 `generic.image` 或通过省略字段绕过资产图 policy。真正与剧本资产总表无关的通用图片仍可独立使用 `generic.image`，但不属于该资产 Task。文本结果以 `single|candidates` 分支表达；异步媒体的全部新创作参数只存在于 `request.kind=new`，`request.kind=retry` 只接受失败 Resource identity。Agent-facing 图片/视频/音乐字段使用跨 provider 产品名，且不得包含 `modelKey`、`*Model` 或 provider 参数；实际模型由服务端正式配置唯一解析。`generationOptions` 只是在批准计划与 Revision provenance 中冻结的内部执行快照，不是公开 Tool 参数。服务端解析出的 modelKey 对应 capability registry 是允许字段和值的唯一裁判，不支持必须 typed-fail，禁止跨模型默认或 provider 参数透传。
...
- **CR-15 — 剧本与风格采用使用保留 Binding。** 当前确认剧本只由 `confirm_script_resource` 写入当前 project/episode canonical scope 的 `role=confirmed_screenplay + slotKey=primary`；当前正式风格只由 `adopt_style_bible` 写入同一 scope 的 `role=adopted_style_bible + slotKey=primary`。两者都绑定精确不可变 Revision，不能从最近文字 Resource、历史消息、Canvas 位置或旧专业表猜测。episode 优先、project fallback 的 canonical Binding scope 顺序只由共享 identity resolver 声明，工作集读取与事务锁定必须共同复用，不能各写一份优先级；两侧还必须经同一个 stored-scope parser 校验 `scopeKind/scopeId/userId/projectId/episodeId` 完整 tuple，旧数据冗余列不一致时原地失败。通用 `adopt_resource` 必须拒绝这两个保留角色，避免第二 writer。`confirm_script_resource` 还必须拒绝 raw text、generic text、非 structured content、非 `materialize_screenplay_draft` origin、Resource/Task/目标 scope 不一致、缺失 completed `creative_work_v4` Task provenance 或不能通过当前 `screenplayResourceDocumentSchema` 的 Revision；它必须从该 Task 的 strict result 重新运行权威 07S compiler，要求重算文档与待确认 Revision 完全一致，并在同一事务锁定当前 scope 的 adopted Style Binding，拒绝剧本 Style tuple 已陈旧或缺失。确认与采用不复制 Resource，也不新建同内容剧本。
- **CR-15A — 资产枚举锁定确认剧本与同一风格。** `asset_prompt_set` 的 production context 只能由服务端同时读取 `confirmed_screenplay` 与 `adopted_style_bible` 保留 Binding，校验两个精确 ready Revision、结构化 screenplay schema，以及 screenplay 内 `source.styleRevision` 与当前 adopted Style 的 resource/revision/fingerprint/bindingVersion/schema tuple 一致后冻结。确认剧本的 `canonicalRegistries.characters/locations/props` 是资产身份的唯一穷尽总表；Worker 必须逐项且仅逐项各输出一次，不能从可读剧本文本、历史消息、名称或 Canvas 再推断资产。该 Task 的结果仍只是创意 Prompt 集，不自动创建图片 Resource 或改写剧本/风格 Binding。
...
旧 `ProjectEditBible`、`ProjectEditSourceScript` 及其 `script_review/approve_script` 链仍服务遗留专业卡片，但不拥有 `confirmed_screenplay` 保留 Binding，也不能冒充 `materialize_screenplay_draft` 来源。该旧链尚未删除，因此剧本 Resource 层仍有残余领域双轨，只能称新 Resource-native 链局部收敛，不能称架构完成。
...
| 结构化 screenplay Revision 与 07S scene instances | `materialize_screenplay_draft` transactional Operation；输入仅为 completed `screenplay_draft` Task | 用户审阅、`confirm_script_resource`、专业 renderer |
...
- 剧本 strict 文档与 07S compiler：`src/lib/creative-worker/screenplay-contract.ts`；结构化 Revision 唯一 materializer：`src/lib/operations/domains/assistant/creative-screenplay-ops.ts` 的 `materialize_screenplay_draft`，它以经校验的 Task episode scope 同时驱动 Resource 写入和 Operation 影响范围。
- Resource-native 剧本确认与保留角色约束：`src/lib/operations/domains/creative-resource/resource-ops.ts` 的 `confirm_script_resource`、`src/lib/creative-resource/contracts.ts`；该 Operation 必须在写 Binding 前重读 completed Task、核对 Task/Resource/目标 scope 与 origin，并从 Task result 重算 07S 后精确比较文档。通用采用入口不能写保留角色，旧 `ProjectEditBible` 确认不能写该 Binding。
- screenplay 来源 Revision、确认剧本与 adopted Style 的联合读取、schema/lineage 校验和 production context 冻结：`src/lib/operations/domains/assistant/creative-production-context.ts`；调用方不得各自从 Resource head、历史消息或自报 content 拼接来源。Resource 来源只接受同 owner 的 user scope、同 project 的 project scope 或同 episode 的 episode scope，并只冻结 `contentText/contentJson`；domain provenance 在没有正式 resolver 时显式失败，第二份 Style Bible Resource 不能伪装成普通剧本来源。
...
- 数据表：`prisma/schema.prisma` 的 `CreativeResource*`；`20260717120000_add_creative_resource_spine` 创建 Resource 脊柱，`20260720190000_add_creative_resource_data` additive 增加开放创作文档及 CAS version。
- Style Bible 资产图构图切换：停止新 Task 提交并排空相关 queued/processing Task 后，`prisma/migrations/20260722090000_remove_asset_style_composition/migration.sql` 只清理可变专业当前态；`scripts/migrations/remove-asset-style-composition.ts` 再为当前 `project.style_bible` head 与正式采用 Binding 指向的旧 revision 追加去掉资产图 `composition` 的新不可变 successor，并用生产 fingerprint/persistence 原子推进当前指针。脚本默认 dry-run，只有显式 `--apply` 才写入；不得原地改写旧 Revision、completed Task、Lineage、creativeData 或其精确引用。
...
- `tests/unit/creative-worker/screenplay-compiler.test.ts` 反证 07S 跨越中间场景错误合并、kernel payload 丢失或持久投影篡改，并覆盖适用的 06T registry、跨层 chronology、canonical name、ledger mutation、replay/hook 与 kernel 引用一致性负例；`tests/contracts/project-agent-toolset-conformance.test.ts` 只锁定 `creative_work_v4`、chapter output policy、专用 materialize/confirm 入口和保留 Binding 的 Tool 契约。来源 Revision canonicalize、materialize episode scope、confirm Task/Resource origin/scope 与 07S 重算没有专门执行路径证据；真实 `screenplay_draft → materialize → confirm → asset_prompt_set` 组合仍未由现有 Golden 完整证明。
...
- `project.source_script` v4 初版只要求 completed Task、同 project 和可解析 07S 文档，并把调用方自报的来源 content/provenance 当作 lineage；确认入口也只检查 operation marker，没有重新证明 Task/Resource/episode scope 或文档确由该 Task 确定性生成。结果是跨 episode 物化、任意内容贴 Resource/domain lineage、或伪造 materializer metadata 都可能通过局部检查。当前 production-context compiler 重读 Resource 的精确 Revision/fingerprint/scope 与真实 text/JSON，无 resolver 的 domain provenance fail closed；materializer 复用精确 Task scope，confirm 重读 Task/Resource origin/scope 并重算 07S 后才写 Binding。专门 execute-path/Critical 与完整 Golden 仍是未验证盲区。

- 上述 confirm 重算的第一版仍只证明“剧本沿用了生成时的 Style”，没有证明它仍等于确认瞬间的 canonical adopted Style；用户在物化后切换风格时，旧剧本可以先被确认，直到资产委派才暴露不一致。当前唯一确认 writer 在事务内按 episode→project 优先级锁定 adopted Style Binding（包括缺失行的 canonical key），并逐字段核对 resource/revision/fingerprint/bindingVersion/schema tuple 后才写 confirmed screenplay；真实并发切换与确认组合仍缺 Critical 证据。

- Resource-native 剧本确认第一次只建立精确 Revision Binding，却仍允许“任意 ready `project.source_script`”被解释为正式剧本，完整 canonical registries、Style lineage 与 Task provenance 没有唯一采用入口；同时旧 `ProjectEditBible` 确认继续存在，形成两个正式剧本候选 writer。当前防线新增唯一 `materialize_screenplay_draft`，用确定性 07S 把 completed v4 Task 写成结构化 Revision，并让 `confirm_script_resource` 拒绝非 structured、非该 Operation/Task 来源；资产输入只从确认 Binding 与同一 Style Binding冻结。旧 `ProjectEditBible` 链仍是显式残余，未删除前不得宣称剧本事实已全仓统一。
